...
All IDs in DB are integers (SERIAL / autoincrement).
"""
...
def _get_or_create_company(tenant_id: str, conn) -> int:
    """
    Return the Company.id (int) for tenant_id.
    Creates the company row automatically if it does not exist.
    """
    with conn.cursor() as cur:
        cur.execute(
            'SELECT id FROM "Company" WHERE slug = %s',
            (tenant_id,),
        )
        row = cur.fetchone()
    ...
    with conn.cursor() as cur:
        cur.execute(
            'INSERT INTO "Company" (name, slug) VALUES (%s, %s) RETURNING id',
            (tenant_id, tenant_id),
        )
        new_id = cur.fetchone()[0]
    conn.commit()

    logger.info("Auto-created company: slug=%s id=%d", tenant_id, new_id)
    return new_id


def _get_document(company_id: int, document_id: str, conn) -> dict | None:
    """
...
def ingest_document(
    ...
    with get_conn() as conn:
        ...
        if existing_doc:
            # ── APPEND branch ─────────────────────────────────────────────────
            db_document_id: int = existing_doc["id"]
            ...
            with conn.cursor() as cur:
                for p in extracted:
                    ...
                    cur.execute(
                        """
                        INSERT INTO "Page"
                            ("documentId", "pageNumber", content, "tokenCount")
                        VALUES (%s, %s, %s, %s)
                ...
                cur.execute(
                    'UPDATE "Document" SET "totalPages" = %s WHERE id = %s',
                    (new_total, db_document_id),
                )
            conn.commit()

            logger.info(
                "Appended %d pages to doc id=%d (was %d, now %d).",
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO "Document"
                        ("companyId", title, "sourceFile", "totalPages")
                    VALUES (%s, %s, %s, %s)
                    RETURNING id
                    """,
            ...
            logger.info(
                "Created doc id=%d with %d pages (tenant=%s, doc=%s).",
        ...
        try:
            ...
            logger.exception("Metadata generation failed for doc id=%d.", db_document_id)
            raise HTTPException(
        ...
        try:
            ...
            logger.exception("Tree build failed for doc id=%d.", db_document_id)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
